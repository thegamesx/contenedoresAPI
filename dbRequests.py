...
def name_cont(contID, userID, name):
    db = connect()
    data, count = (db.table("container").
...
def check_defrost_status(cont_id, db):
    data, count = (db.table("signals").
                   select("*", count='exact').
                   order("date", desc=True). #Ver si acomoda los datos de forma apropiada
                   eq("idvigia", cont_id).
                   limit(20).execute())
    inactive_time=0
    for row in data[1]:
...
def controller_status(lastSignal):
    ...
    timeDelta = lastSignalDT-timeNow
...
def status_cont_client(clientID):
    ...
    # Cargar la db con datos de testeo y probar esto luego
